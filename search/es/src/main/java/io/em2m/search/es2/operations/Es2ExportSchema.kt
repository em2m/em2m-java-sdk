...
private fun es2ExportSchema(es2Client: Es2Api,
    ...
    if (indexWhitelist.isNotEmpty()) {
        indices = indices.filter { es2Index: Es2Index -> es2Index.index in indexWhitelist }
        aliases = aliases.filterKeys { index: String -> index in indexWhitelist }
        mappings = mappings.filterKeys { index: String -> index in indexWhitelist }
    }
