    ...
    fun convertAggs(aggs: List<Agg>, params: Map<String, Any>): EsAggs {
        ...
        aggs.forEach {
            when (it) {
                ...
                is HistogramAgg -> {
                    result.histogram(it.key, it.field, it.interval, it.offset).minDocCount(it.minDocCount)
                }
                is StatsAgg -> {
                    result.stats(it.key, it.field).minDocCount(it.minDocCount)
                }
                is DateHistogramAgg -> {
                    result.dateHistogram(it.key, it.field, it.format, it.interval, it.offset, timeZone.id).minDocCount(it.minDocCount)
                }
                is RangeAgg -> {
                    val esRanges = result.agg(it.key, "range").put("field", it.field).minDocCount(it.minDocCount)
                            .withArray("ranges")
                    it.ranges.forEach {
                        esRanges.addPOJO(it)
                    }
                ...
                is DateRangeAgg -> {
                    val esAgg = result.agg(it.key, "date_range").put("field", it.field).minDocCount(it.minDocCount)
                ...
                is GeoHashAgg -> {
                    val esAgg = result.agg(it.key, "geohash_grid").put("field", it.field).minDocCount(it.minDocCount)
                ...
                is GeoCentroidAgg -> {
                    result.agg(it.key, "geo_centroid").put("field", it.field).minDocCount(it.minDocCount)
                }
                is GeoBoundsAgg -> {
                    result.agg(it.key, "geo_bounds").put("field", it.field).minDocCount(it.minDocCount)
                }
                is GeoDistanceAgg -> {
                    val esAgg = result.agg(it.key, "geo_distance").put("field", it.field).minDocCount(it.minDocCount)
