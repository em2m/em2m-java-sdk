...
class MigrateByQueryEs2ToEs8Test {
    ...
    fun `migrate es2 to es8 by query`() {
        ...
        es2Schema.aliasesToMappings.forEach { (alias, mappings) ->
            ...
            if (index !in indexWhitelist && indexWhitelist.isNotEmpty()) return@forEach //overkill bc in the export schema call
            ...
            try {
                ...
                if (indexExists && "localhost:9200" in es8Url) {
        ...
        unprocessedIndices.forEach { index ->
            if (index !in indexWhitelist && indexWhitelist.isNotEmpty()) return@forEach
            if (ignorePatterns.any { it(index) }) return@forEach
