...
class MapBackedDaoAggTest : Assert() {
    ...
    fun before() {
        ...
        searchDao = MapBackedSyncDao(FeatureMapper(), featureMap)
    }
    ...
    fun testHistogram() {
        ...
        val result = searchDao.search(request)
        val histogram = result.aggs["magnitude"] ?: error("agg should not be null")
        val buckets = histogram.buckets ?: error("buckets should not be null")
